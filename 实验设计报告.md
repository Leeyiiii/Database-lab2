...
**用户角色：** **管理员**（拥有全部后台管理权限）与 **学生**（自助查询与申请）。

系统共包含 **8 个功能模块、45 项功能需求**，覆盖如下领域：
...
| 学生自助服务 | 5 | 个人首页仪表盘、课程浏览与选退课、转专业自助申请、成绩/GPA 查询、奖惩记录查看 |
| 合计 | 45 | 图片、视频、文件全覆盖，双角色支持 |
...
| F-39 | 账号管理 | 管理员账号增删改查 |
| F-40 | 密码修改 | 管理员修改登录密码 |
| F-41 | 学生登录 | 学生凭学号和密码登录系统 |
| F-42 | 个人首页仪表盘 | 查看个人信息概览、当前专业、选课与奖惩统计 |
| F-43 | 学生自助选退课 | 浏览全部课程、自助选课与退课 |
| F-44 | 学生成绩查询 | 查看个人全部课程成绩与 GPA |
| F-45 | 学生奖惩查询 | 查看个人奖惩记录 |
...
- **模块化路由**：使用 Flask Blueprint 将系统拆分为 **7 个独立功能模块**（auth、student、major、reward_punish、course、score/enrollment、portal），每个模块内聚度高、耦合度低。其中 `portal` 为新增的学生自助服务模块
- **双角色认证**：登录时根据用户选择的角色，分别查询 Admin 表或 Student 表进行身份验证，管理员和学生拥有完全不同的界面和权限
...
├── blueprints/
│   ├── __init__.py
│   ├── auth.py               # 登录/登出、管理员 CRUD、密码修改（含双角色认证）
...
│   └── student_portal.py     # 学生端：首页仪表盘、自助选退课、转专业申请
├── templates/
│   ├── base.html             # 基础模板（管理员端导航栏 + 侧边栏 + Bootstrap 样式）
│   ├── login.html            # 登录页（双角色选择卡片：管理员/学生）
...
│   │   # --- 学生端模板 ---
│   ├── student_base.html     # 学生端基础模板（简洁导航栏）
│   ├── student_dashboard.html    # 学生个人首页仪表盘
│   ├── student_courses.html      # 浏览全部课程 + 选课/退课
│   ├── student_scores.html       # 查看个人成绩与 GPA
│   ├── student_rewards.html      # 查看个人奖惩记录
│   └── student_major_change.html # 自助转专业申请
└── static/
    └── uploads/              # 上传文件存储目录
```
...
> **设计要点：** 使用 Flask Blueprint 实现模块化路由，7 个功能模块独立注册。根路径 `/` 根据登录角色智能跳转——管理员进入学生列表，学生进入个人仪表盘，未登录则进入登录页。应用启动时自动创建上传目录。
...
#### 2.3.5 双角色登录认证 — 管理员与学生分表验证

系统支持**管理员**和**学生**两类用户登录。登录时由 `role` 参数决定查询哪张表，`session` 中保存用户身份信息，后续路由根据 `session['role']` 区分权限。
...
def login():
    if request.method == 'POST':
        ...
        role = request.form.get('role', 'admin')  # 'admin' 或 'student'
        ...
        if role == 'admin':
            ...
            # ========== 学生登录（查 Student 表）==========
            row = query(
                'SELECT student_id, name, password_hash FROM Student '
...
> **设计要点：**
> - **分表认证**：管理员查 `Admin` 表（按用户名），学生查 `Student` 表（按学号）。两类用户完全隔离，互不干扰
> - **退学学生拦截**：学生登录 SQL 包含 `is_deleted=0` 条件，已退学（软删除）的学生无法登录
> - **角色标记**：`session['role']` 贯穿全局，管理员端和学生端路由通过检查此值实现权限隔离
> - **前端区分**：登录页提供"管理员"和"学生"两个角色选择卡片，用户点击切换后提交不同的 `role` 值

#### 2.3.6 学生端核心路由 — 自助选退课 + 转专业申请

学生端 `portal` Blueprint 提供 5 个功能入口，均受 `student_required()` 装饰器保护：
...
def courses():
    ...
    # 已选课程ID集合（用于前端标记"已选"状态）
    enrolled = query('SELECT course_id FROM Enrollment WHERE student_id=%s', (sid,), fetchall=True)
    enrolled_set = {row[0] for row in enrolled}
    return render_template('student_courses.html',
                           courses=all_courses, enrolled_set=enrolled_set)
...
def drop(enrollment_id):
    sid = session['student_id']
    row = query('SELECT course_id FROM Enrollment WHERE enrollment_id=%s AND student_id=%s',
...
> **说明：** 选课时自动从课程表获取学期信息，利用 `(student_id, course_id, semester)` UNIQUE 约束防止重复选课（异常时提示"可能已选过"）。退课时先验证 `enrollment_id` 属于当前学生，防止越权操作。
...
def major_change():
    ...
    if request.method == 'POST':
        ...
        if new_major_id == current_major_id:
            ...
            query('''INSERT INTO MajorChange (student_id, old_major_id, new_major_id,
                       change_date, reason, status) VALUES (%s,%s,%s,%s,%s,'待审批')''',
    ...
    history = query('''SELECT mc.*, m1.major_name AS old_name, m2.major_name AS new_name
        FROM MajorChange mc LEFT JOIN Major m1 ON mc.old_major_id=m1.major_id
        LEFT JOIN Major m2 ON mc.new_major_id=m2.major_id
        WHERE mc.student_id=%s ORDER BY mc.change_date DESC''', (sid,), fetchall=True)
    return render_template('student_major_change.html',
                           all_majors=all_majors, current_major_id=current_major_id, history=history)
```

> **说明：** 学生端提交的变更记录默认状态为"待审批"，由管理员在后台审批。审批通过后，`trg_major_change_sync` 触发器自动同步学生表的 `major_id` 字段。
...
![1781249006688](image/实验设计报告/1781249006688.png)
...
![1781249021633](image/实验设计报告/1781249021633.png)

### 3.18 学生端 — 个人首页仪表盘


![1781249064332](image/实验设计报告/1781249064332.png)

### 3.19 学生端 — 课程浏览与选退课


![1781249090172](image/实验设计报告/1781249090172.png)

### 3.20 学生端 — 成绩与 GPA 查询


![1781249106541](image/实验设计报告/1781249106541.png)

### 3.21 学生端 — 自助转专业申请

![1781249124947](image/实验设计报告/1781249124947.png)
